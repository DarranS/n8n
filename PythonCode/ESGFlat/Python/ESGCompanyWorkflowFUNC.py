...
def generate_markdown_report(
    ...
    # Header with Company Name and LGT Sustainability Rating
    lgt_rating = esg_data[0].get('LGTRating', '') if esg_data else ''
    md_content.append(f"# ESG Report")
    md_content.append(f"**Company Name**: {company_name}")
    md_content.append(f"**LGT Sustainability Rating**: {lgt_rating}")
    md_content.append("")
...
def main(company_data: str):
    ...
    try:
        ...
        lgt_rating = data.get('lgtSustainabilityRating', '')

        # Process all data in memory
        esg_data = process_esg_data(data)
        # Add LGTRating to esg_data for access in generate_markdown_report
        for row in esg_data:
            row['LGTRating'] = lgt_rating
